...
function publish(config) {
  return new Promise((resolve, reject) => {
    ...
    deployh5.publish(basePath, config, err => {
      if (err) {
        return reject(err);
      }
      resolve();
  ...
  return Promise.resolve().then(() => {
    program
      ...
      .option(
        '-m, --message <message>',
        'commit message',
        deployh5.defaults.message
      )
      .option('-g, --tag <tag>', 'add tag to commit')
      .option('--git <git>', 'Path to git executable', deployh5.defaults.git)
      .option('-t, --dotfiles', 'Include dotfiles')
      .option('-r, --repo <repo>', 'URL of the repository you are pushing to')
      .option('-p, --depth <depth>', 'depth for clone', deployh5.defaults.depth)
      .option(
        '-o, --remote <name>',
        'The name of the remote',
        deployh5.defaults.remote
      )
      .option(
        '-u, --user <address>',
