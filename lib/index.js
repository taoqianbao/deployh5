...
const log = util.debuglog('deployh5');
...
function getCacheDir(optPath) {
  const dir = findCacheDir({name: 'deployh5'});
  if (!optPath) {
    return dir;
  }
...
/**
 * Push a git branch to a remote (pushes deployh5 by default).
        ...
        return git.getRemoteUrl(options.remote).then(url => {
          if (url !== repoUrl) {
            const message =
              ...
              '" in ' +
              git.cwd +
              '.  Try running the `deployh5-clean` script first.';
