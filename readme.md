...
Publish files to a `build` branch on GitHub (or any other branch anywhere else).

## Getting Started

```shell
npm install deployh5 --save-dev
```
...
```js
var deployh5 = require('deployh5');

deployh5.publish('dist', function(err) {});
```
...
```js
deployh5.publish(dir, callback);
// or...
deployh5.publish(dir, options, callback);
```

Calling this function will create a temporary clone of the current repository, create a `deployh5` branch if one doesn't already exist, copy over all files from the base path, or only those that match patterns from the optional `src` configuration, commit all changes, and push to the `origin` remote.

If a `deployh5` branch already exists, it will be updated with all commits from the remote before adding any commits from the provided `src` files.

**Note** that any files in the `deployh5` branch that are *not* in the `src` files **will be removed**.  See the [`add` option](#optionsadd) if you don't want any of the existing files removed.
 ...
 *       site.js
 *
 * The usage below will create a `deployh5` branch that looks like this:
 *
 *   index.html
 *   js/
 *     site.js
 *
 */
deployh5.publish('dist', callback);
```
...
The name of the branch you'll be pushing to.  The default uses GitHub's `deployh5` branch, but this can be configured to push to any branch on any remote.
 ...
 */
deployh5.publish('dist', {dotfiles: true}, callback);
```
...
```js
/**
 * The usage below will only add files to the `deployh5` branch, never removing
 * any existing files (even if they don't exist in the `src` config).
 */
deployh5.publish('dist', {add: true}, callback);
```
...
By default, `deployh5` assumes that the current working directory is a git repository, and that you want to push changes to the `origin` remote.
 ...
 * push all files in the `src` config to the `deployh5` branch of the `repo`.
 */
deployh5.publish('dist', {
  repo: 'https://example.com/other/repo.git'
}, callback);
...
```js
/**
 * This task pushes to the `deployh5` branch of of your `upstream` remote.
 */
deployh5.publish('dist', {
  remote: 'upstream'
}, callback);
...
`--add`). By default, `deployh5` removes everything inside the target branch
...
```js
deployh5.publish('dist', {push: false}, callback);
```
...
```js
deployh5.publish('dist', {history: false}, callback);
```
...
```bash
deployh5 --before-add ./cleanup.js
```
 ...
 * Assuming we want to keep everything on the deployh5 branch
...
Installing the package creates a `deployh5` command line utility.  Run `deployh5 --help` to see a list of supported options.

With a local install of `deployh5`, you can set up a package script with something like the following:

```shell
"scripts": {
  "deploy": "deployh5 -d dist"
}
```

And then to publish everything from your `dist` folder to your `deployh5` branch, you'd run this:
...
To get additional output from the `deployh5` script, set `NODE_DEBUG=deployh5`.  For example:

```shell
NODE_DEBUG=deployh5 npm run deploy
```

## Dependencies

Note that this plugin requires Git 1.9 or higher (because it uses the `--exit-code` option for `git ls-remote`).  If you'd like to see this working with earlier versions of Git, please [open an issue](https://github.com/tschaub/deployh5/issues).

![Test Status](https://github.com/tschaub/deployh5/workflows/Test/badge.svg)

## Tips

### when get error `branch already exists`
```
{ ProcessError: fatal: A branch named 'deployh5' already exists.

    at ChildProcess.<anonymous> (~/node_modules/deployh5/lib/git.js:42:16)
    at ChildProcess.emit (events.js:180:13)
    at maybeClose (internal/child_process.js:936:16)
    at Process.ChildProcess._handle.onexit (internal/child_process.js:220:5)
  code: 128,
  message: 'fatal: A branch named \'deployh5\' already exists.\n',
  name: 'ProcessError' }
  ```

The `deployh5` module writes temporary files to a `node_modules/.cache/deployh5` directory.  The location of this directory can be customized by setting the `CACHE_DIR` environment variable.

If `deployh5` fails, you may find that you need to manually clean up the cache directory.  To remove the cache directory, run `node_modules/deployh5/bin/deployh5-clean` or remove `node_modules/.cache/deployh5`.
...
```
echo 'your_cutom_domain.online' > ./build/CNAME && deployh5 -d build"
```
...
```yaml
- name: Deploy with deployh5
  run: |
    git remote set-url origin https://git:${GITHUB_TOKEN}@github.com/${GITHUB_REPOSITORY}.git
    npx deployh5 -d build -u "github-actions-bot <support+actions@github.com>"
   env:
...
See [Issue #345](https://github.com/tschaub/deployh5/issues/345) for more information

#### Deploying with GitHub Actions and a named script

If you are using a named script in the `package.json` file to deploy, you will need to ensure you pass the variables properly to the wrapped `deployh5` script. Given the `package.json` script below:

```json
"scripts": {
  "deploy": "deployh5 -d build"
}
```
...
See [Pull Request #368](https://github.com/tschaub/deployh5/pull/368) for more information.
